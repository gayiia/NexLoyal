{{-- This view edits an existing mystery box configuration. --}}
<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'NexLoyal') }} - Edit Mystery Box</title>
            ...
            <div class="min-h-screen bg-[linear-gradient(120deg,rgba(15,23,42,0.9),rgba(2,6,23,0.95))] nl-shell">
                <div class="flex min-h-screen flex-col lg:flex-row">
                    ...
                    <main class="flex-1 px-4 py-6 sm:px-6 lg:px-10 lg:py-8">
                        {{-- The header anchors the edit flow and provides a return link. --}}
                        <x-page-header eyebrow="" title="Edit Mystery Box" breadcrumb="Coupons / Mystery Box">
                            <x-slot name="actions">
                        ...
                        {{-- This form updates the mystery box configuration in the backend. --}}
                        <form method="POST" action="{{ route('mystery-boxes.update', $mysteryBox) }}" class="mt-6 space-y-6">
                            @csrf
                            {{-- PATCH signals a partial update for the existing record. --}}
                            @method('PATCH')
                            <section class="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 nl-panel">
                                ...
                                <div class="mt-4 grid gap-4 md:grid-cols-2">
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <input class="rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-sm text-slate-200" type="text" name="name" value="{{ old('name', $mysteryBox->name) }}" required>
                                        @error('name')
                                    ...
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <select class="rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-sm text-slate-200" name="claim_rule" required>
                                            @php
                                                // Preserve the existing rule or the user's previous input.
                                                $rule = old('claim_rule', $mysteryBox->claim_rule);
                                            @endphp
                                            <option value="ONCE_PER_DAY" @selected($rule === 'ONCE_PER_DAY')>Once per day</option>
                                            <option value="ONCE_PER_WEEK" @selected($rule === 'ONCE_PER_WEEK')>Once per week</option>
                                            <option value="ONCE_EVER" @selected($rule === 'ONCE_EVER')>Once ever</option>
                                        </select>
                                        @error('claim_rule')
                                            <p class="text-xs text-rose-300">{{ $message }}</p>
                                        @enderror
                                    ...
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <input class="rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-sm text-slate-200" type="date" name="starts_at" value="{{ old('starts_at', optional($mysteryBox->starts_at)->format('Y-m-d')) }}">
                                        @error('starts_at')
                                            <p class="text-xs text-rose-300">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <input class="rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-sm text-slate-200" type="date" name="ends_at" value="{{ old('ends_at', optional($mysteryBox->ends_at)->format('Y-m-d')) }}">
                                        @error('ends_at')
                                            <p class="text-xs text-rose-300">{{ $message }}</p>
                                        @enderror
                                    </div>
                                ...
                                <div class="mt-4 grid gap-3 md:grid-cols-2">
                                    ...
                                    @foreach ($tiers as $tier)
                                        <label class="flex items-center gap-3 rounded-lg border border-slate-800 px-3 py-2 text-sm text-slate-200">
                                            <input type="checkbox" name="tiers[]" value="{{ $tier->id }}" @checked(in_array($tier->id, old('tiers', $mysteryBox->tiers ?? []), true))>
                            ...
                            <section class="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 nl-panel">
                                ...
                                <div class="mt-4 grid gap-3 md:grid-cols-2">
                                    ...
                                    @forelse ($coupons as $coupon)
                                        <label class="flex items-center gap-3 rounded-lg border border-slate-800 px-3 py-2 text-sm text-slate-200">
                                            <input type="checkbox" name="coupons[]" value="{{ $coupon->id }}" @checked(in_array($coupon->id, old('coupons', $selectedCoupons), true))>
                            ...
                            <div class="flex flex-wrap justify-end gap-3">
                                ...
                                {{-- Submit saves the updated mystery box configuration. --}}
