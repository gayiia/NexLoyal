...
{{-- This view renders the form used to create a draft AI award tied to a customer cluster. --}}
<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'NexLoyal') }} - Create AI Award</title>
                ...
                <div class="min-h-screen bg-[linear-gradient(120deg,rgba(15,23,42,0.9),rgba(2,6,23,0.95))] nl-shell">
                    <div class="flex min-h-screen">
                        ...
                        <main class="flex-1 px-10 py-8">
                            {{-- The header gives context and quick navigation back to AI insights. --}}
                            <x-page-header eyebrow="AI Insights" title="Create award" subtitle="Draft a smart offer" breadcrumb="AI Insights / Awards">
                                <x-slot name="actions">
                            ...
                            {{-- This form posts a new award draft so it can be reviewed before issuance. --}}
                            <form method="POST" action="{{ route('ai-insights.awards.store') }}" class="mt-6 rounded-2xl border border-slate-800 bg-slate-900/70 nl-panel p-6 space-y-6">
                                @csrf
                                ...
                                <div class="grid gap-4 md:grid-cols-2">
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <input class="rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-sm text-slate-200" type="text" name="title" value="{{ old('title') }}" required>
                                        @error('title')
                                    ...
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <select class="rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-sm text-slate-200" name="ai_cluster_id" required>
                                            <option value="" disabled @selected(!old('ai_cluster_id'))>Select cluster</option>
                                            @foreach ($clusters as $cluster)
                                                <option value="{{ $cluster->id }}" @selected((string) old('ai_cluster_id') === (string) $cluster->id)>{{ $cluster->label }}</option>
                                            @endforeach
                                        </select>
                                        @error('ai_cluster_id')
                                            <p class="text-xs text-rose-300">{{ $message }}</p>
                                    ...
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <select class="rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-sm text-slate-200" name="type" id="award-type" required>
                                            <option value="points" @selected(old('type', 'points') === 'points')>Points</option>
                                            <option value="coupon" @selected(old('type') === 'coupon')>Coupon</option>
                                        </select>
                                    </div>
                                    <div class="flex flex-col gap-2" data-type-section="points">
                                        ...
                                        <input class="rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-sm text-slate-200" type="number" min="1" name="points_amount" value="{{ old('points_amount') }}">
                                        @error('points_amount')
                                            <p class="text-xs text-rose-300">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div class="flex flex-col gap-2" data-type-section="coupon">
                                        ...
                                        <select class="rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-sm text-slate-200" name="coupon_id">
                                            <option value="" disabled @selected(!old('coupon_id'))>Select coupon</option>
                                            @foreach ($coupons as $coupon)
                                                <option value="{{ $coupon->id }}" @selected((string) old('coupon_id') === (string) $coupon->id)>{{ $coupon->title }}</option>
                                            @endforeach
                                        </select>
                                        @error('coupon_id')
                                            <p class="text-xs text-rose-300">{{ $message }}</p>
                                ...
                                {{-- When clustering has not been run, awards cannot be meaningfully targeted. --}}
                                @if ($clusters->isEmpty())
                                    <div class="rounded-xl border border-slate-800 bg-slate-900/60 px-4 py-3 text-xs text-slate-400">
                                        Run clustering before creating awards.
                                    </div>
                                @endif
